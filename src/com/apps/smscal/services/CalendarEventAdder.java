...
public class CalendarEventAdder implements EventAdder {
    ...
    public CalendarEventAdder(CalendarInfo info, ContentResolver resolver) {
        this.calendarInfo = info;
        this.contentResolver = resolver;
    }

    /* (non-Javadoc)
     * @see com.apps.smscal.services.EventAdder#add(com.apps.smscal.model.EventInfo)
     */
    @Override
    public void add(EventInfo info) {
        ...
        values.put(Events.CALENDAR_ID, this.calendarInfo.getId());
        values.put(Events.EVENT_TIMEZONE, info.getTimeZone());

        this.contentResolver.insert(Events.CONTENT_URI, values);
    }
}
